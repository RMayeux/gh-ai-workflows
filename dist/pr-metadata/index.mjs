import{a as e,c as t,f as n,h as r,l as i,m as a,n as o,o as s,p as c,r as l,s as u,t as d,u as f}from"../llm-wSWP-siq.mjs";const p=n({title:c().min(1,`Title is required`).max(72,`Title must be under 72 characters`),body:c().min(1,`Body is required`),change_type:t([`feat`,`fix`,`refactor`,`perf`,`docs`,`test`,`build`,`ci`,`chore`]),breaking:f().default(!1),doc_impact:f().default(!1),doc_slugs:i(c()).default([])}),m={id:`pr-metadata`,system:`You are a staff-level engineer analyzing a pull request.
...
# CODE DIFF
{{code_diff}}`,overrides:{}};async function h(t){let{githubToken:n,llm:i,model:c,apiKey:f,owner:h,repo:g,pullNumber:_,maxTokens:v=4096,debug:y=!1,githubClient:b}=t;y&&u.debug(`Running PR Metadata Workflow for ${h}/${g}#${_}`);try{u.log(`Step 1: Initializing GitHub Client...`);let t=b||new o(n),x=new r(t);u.log(`Step 2: Fetching PR diff and files...`);let S=await x.buildPRContext(h,g,_).catch(e=>{throw Error(`Failed to build PR context: ${e.message}`)});u.log(`Step 3: Loading and rendering prompt...`);let C=e.render(m,{registry:``,changed_files:S.files.join(`\\n`),code_diff:S.diff,pr_title:S.details.title,pr_body:S.details.body??``});u.log(`Step 4: Generating metadata using ${i}:${c}...`),d();let w=await s(a.create(i,{apiKey:f,model:c}),p,{prompt:C.user,systemPrompt:C.system,maxTokens:v},{maxRetries:3,jsonMode:!0}).catch(e=>{throw Error(`LLM request failed: ${e.message}`)});if(!w.success)throw Error(`LLM Generation failed: ${w.error}`);let T=w.data;y&&u.debug(`Generated Metadata:`,T),u.log(`Step 5: Updating PR title, body and labels...`),await t.updatePR(h,g,_,T.title,T.body).catch(e=>{throw Error(`Failed to update PR: ${e.message}`)});let E=[];T.change_type&&E.push(T.change_type),T.breaking&&E.push(`breaking-change`),T.doc_impact&&E.push(`doc-impact`);let D=S.details.additions+S.details.deletions;return D<50?E.push(`size/XS`):D<200?E.push(`size/S`):D<500?E.push(`size/M`):D<1e3?E.push(`size/L`):E.push(`size/XL`),await l(t,h,g,_,{add:E}),u.log(`PR Metadata Workflow completed successfully.`),T}catch(e){let t=e instanceof Error?e.message:String(e);throw u.error(`Workflow failed at step: ${t}`),e}}async function g(){let e={GITHUB_TOKEN:process.env.GITHUB_TOKEN,LLM:process.env.LLM,MODEL:process.env.MODEL,API_KEY:process.env.API_KEY,GITHUB_REPOSITORY_OWNER:process.env.GITHUB_REPOSITORY_OWNER,GITHUB_REPOSITORY_NAME:process.env.GITHUB_REPOSITORY_NAME,GITHUB_EVENT_PULL_REQUEST_NUMBER:process.env.GITHUB_EVENT_PULL_REQUEST_NUMBER},t=Object.entries(e).filter(([e,t])=>!t).map(([e])=>e);t.length>0&&(console.error(`Missing required environment variables:`),console.error(t.join(`, `)),process.exit(1));let n={githubToken:process.env.GITHUB_TOKEN||``,llm:process.env.LLM||``,model:process.env.MODEL||``,apiKey:process.env.API_KEY||``,owner:process.env.GITHUB_REPOSITORY_OWNER||``,repo:process.env.GITHUB_REPOSITORY_NAME||``,pullNumber:parseInt(process.env.GITHUB_EVENT_PULL_REQUEST_NUMBER||`0`,10),maxTokens:process.env.MAX_TOKENS?parseInt(process.env.MAX_TOKENS,10):4096,debug:process.env.DEBUG===`true`};try{await h(n),process.exit(0)}catch(e){console.error(`Workflow failed:`,e),process.exit(1)}}process.env.NODE_ENV!==`test`&&g();export{h as runPRMetadataWorkflow};