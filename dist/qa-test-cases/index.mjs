import{c as e,d as t,h as n,i as r,l as i,m as a,n as o,o as s,p as c,r as l,s as u,t as d,u as f}from"../llm-DwD2cvkQ.mjs";import{t as p}from"../file-system-DdvyYQ2J.mjs";import{t as m}from"../date-BLOVFp3M.mjs";const h=i({summary:f().describe(`One sentence describing what changed and why it matters to QA`),impactedFeatures:s(i({featureSlug:f().describe(`The domain/feature-slug`),testCases:s(f()).describe(`List of test cases in "condition → action → expected result" format`)})).optional().default([]),unchangedTestCases:s(f()).optional().default([]).describe(`Test cases from the previous comment that are still valid, copied verbatim`),retiredTestCases:s(f()).optional().default([]).describe(`Test cases from the previous comment that are no longer relevant, copied verbatim`),totalTests:e().describe(`Total number of active test cases (impacted + unchanged, excluding retired)`)});i({githubToken:f().min(1),llm:f().min(1),model:f().min(1),apiKey:f().min(1),owner:f().min(1),repo:f().min(1),pullNumber:e().int().positive(),projectContext:f().optional().describe(`General context about the project to help the AI understand the domain.`),docPattern:f().optional().describe(`Optional regex to find documentation files in the repository. If provided, all matching files will be included in the prompt.`),debug:u().optional()});const g={id:`qa-test-cases`,system:`You are a senior QA lead reviewing a pull request before it goes to QA.
...
`)+`
`),await c(e,_,v,y,`🧪 QA Test Cases`,M).catch(e=>{throw Error(`Failed to post QA comment: ${e.message}`)}),a.log(`QA Test Cases Workflow completed successfully.`),j}catch(e){let t=e instanceof Error?e.message:String(e);throw a.error(`Workflow failed at step: ${t}`),e}}async function v(){let e={githubToken:process.env.GITHUB_TOKEN,llm:process.env.LLM,model:process.env.MODEL,apiKey:process.env.API_KEY,owner:process.env.GITHUB_REPOSITORY_OWNER,repo:process.env.GITHUB_REPOSITORY_NAME,pullNumber:process.env.GITHUB_EVENT_PULL_REQUEST_NUMBER?parseInt(process.env.GITHUB_EVENT_PULL_REQUEST_NUMBER,10):void 0,projectContext:process.env.PROJECT_CONTEXT,docPattern:process.env.DOC_PATTERN,debug:process.env.DEBUG===`true`},t=QATestCasesInputsSchema.safeParse(e);t.success||(console.error(`Invalid or missing environment variables:`),console.error(JSON.stringify(t.error.format(),null,2)),process.exit(1));try{await _(t.data),process.exit(0)}catch(e){console.error(`Workflow failed:`,e),process.exit(1)}}process.env.NODE_ENV!==`test`&&v();export{_ as runQATestCasesWorkflow};