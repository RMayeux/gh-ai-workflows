...
describe('AnthropicProvider', () => {
  ...
  it('throws AuthenticationError on 401', async () => {
    mockFetch.mockResolvedValue(mockErrorResponse(401, { error: { message: 'Invalid creds' } }));
    const provider = createProvider();
    await expect(provider.generate(defaultRequest)).rejects.toThrow(AuthenticationError);
    await expect(provider.generate(defaultRequest)).rejects.toMatchObject({ message: expect.stringContaining('Invalid creds') });
  });
  ...
  it('throws ProviderError on 500', async () => {
    ...
    await expect(provider.generate(defaultRequest)).rejects.toMatchObject({ message: expect.stringContaining('Server error') });
  });
