...
describe('MistralProvider', () => {
  ...
  it('throws AuthenticationError on 401', async () => {
    mockFetch.mockResolvedValue(mockErrorResponse(401, { error: { message: 'Bad auth' } }));
    const provider = createProvider();
    await expect(provider.generate(defaultRequest)).rejects.toThrow(AuthenticationError);
    await expect(provider.generate(defaultRequest)).rejects.toMatchObject({ message: expect.stringContaining('Bad auth') });
  });
